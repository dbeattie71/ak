...
bool HookAPICalls(SDLLHook* Hook, HMODULE hModule)
{
	...
	{
		LPCWSTR pszImportModuleName = MakePtr(LPCWSTR, hModule, pImportDesc->Name);
...
bool UnHookAPICalls( SDLLHook* Hook, HMODULE hModule)
{
	...
	{
		// Get the current protection attributes                            
		VirtualQuery(FHook->OrigPtr, &mbi, sizeof(mbi));
		...
		if (!VirtualProtect(FHook->OrigPtr, (sizeof(PVOID)), flNewProtect, &flOldProtect))
		{
			return false;
		}
		
		memcpy(FHook->OrigPtr,&FHook->OrigFn,sizeof(FHook->OrigFn));//write old address back
		
		// Put the page attributes back the way they were.
		VirtualProtect(FHook->OrigPtr, (sizeof(PVOID)), flOldProtect, &flDontCare);
